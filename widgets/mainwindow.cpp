  ...
  m_useDarkStyle {false}
{
  ...
  loadHralMembers();
  updateHralCheckboxState();
  {
    QString hralPath = m_config.writeable_data_dir().absoluteFilePath("IOTA-HRAL.txt");
    m_hralFileWatcher.addPath(m_config.writeable_data_dir().absolutePath());
    if (QFileInfo::exists(hralPath)) m_hralFileWatcher.addPath(hralPath);
    connect(&m_hralFileWatcher, &QFileSystemWatcher::fileChanged, this, [this]() {
      loadHralMembers();
      updateHralCheckboxState();
    });
    connect(&m_hralFileWatcher, &QFileSystemWatcher::directoryChanged, this, [this, hralPath]() {
      // File may have appeared, disappeared, or been replaced atomically (which severs the file watch).
      if (QFileInfo::exists(hralPath) && !m_hralFileWatcher.files().contains(hralPath)) {
        m_hralFileWatcher.addPath(hralPath);
      }
      loadHralMembers();
      updateHralCheckboxState();
    });
...
{
  LOG_INFO(QString("checked:%1 DX_Call:%2 dblClk:%3").arg(checked).arg(ui->DX_Call_Button->isChecked()).arg(m_bDoubleClicked));
  if (ui->DX_Call_Button->isChecked() && m_specOp==SpecOp::HOUND && m_config.superFox() && !m_bDoubleClicked) return;  // for Wait & Call
  LOG_INFO(QString("no_wait_and_call:%1 wait_and_call:%2").arg(no_wait_and_call).arg(wait_and_call));
  ...
  if (!checked && ui->DX_Call_Button->isChecked()) {
      ...
      no_wait_and_call = false;                             // reset Wait & Call
      LOG_INFO(QString("no_wait_and_call:%1").arg(no_wait_and_call));
  }
  m_specOp=m_config.special_op_id();
...
void MainWindow::readFromStdout()                             //readFromStdout
{
  ...
  while(proc_jt9.canReadLine()) {
    ...
    if (m_mode == "FT8" and m_specOp == SpecOp::FOX and m_ActiveStationsWidget != NULL) { // see if we should add this to ActiveStations window
      // HRAL filter: when active, only let paid-member callers through to the Active Stations window
      bool passesHral = true;
      if (hralFilterActive()) {
        QStringList w = the_line.split(' ', SkipEmptyParts);
        passesHral = (w.size() > 6) && m_hralMembers.contains(Radio::base_callsign(w[6]));
      }
      if (passesHral &&
                            ...
                            , bool /*low_confidence*/, quint8 modifiers)
{
  ...
  auto message_line = format_string
    ...
    .arg (mode, -2)
    .arg (text);
  LOG_INFO(QString("msg is %1").arg(message_line));
  ...
  if (!cursor.isNull ()) 
    {
      LOG_INFO(QString("msg acceptable"));
  ...
  if (parts.size () >= 5)
    {
      ...
      // HRAL filter: when active, only forward decodes whose caller (second token of the message body)
      // is on the paid-member list. PSK Reporter (pskPost) is intentionally NOT filtered here.
      if (hralFilterActive())
        {
          QStringList tokens = decode.mid (has_seconds ? 24 : 22).split (' ', SkipEmptyParts);
          if (tokens.size () < 2) return;
          QString caller = Radio::base_callsign (tokens[1]);
          if (caller.isEmpty () || !m_hralMembers.contains (caller)) return;
        }
...
//------------------------------------------------------------------------------
void MainWindow::loadHralMembers()
{
  m_hralMembers.clear();
  QFile f(m_config.writeable_data_dir().absoluteFilePath("IOTA-HRAL.txt"));
  if (!f.open(QIODevice::ReadOnly | QIODevice::Text)) return;
  QTextStream in(&f);
  while (!in.atEnd()) {
    QString call = Radio::base_callsign(in.readLine().trimmed().toUpper());
    if (!call.isEmpty()) m_hralMembers.insert(call);
  }
  f.close();
}

void MainWindow::updateHralCheckboxState()
{
  bool hasMembers = !m_hralMembers.isEmpty();
  ui->cbHralOnly->setEnabled(hasMembers);
  if (!hasMembers) ui->cbHralOnly->setChecked(false);
}

bool MainWindow::hralFilterActive() const
{
  return m_specOp == SpecOp::FOX
      && ui->cbHralOnly->isChecked()
      && !m_hralMembers.isEmpty();
}
...
// Read and process the file of Hound callers.
    while(!s.atEnd()) {
      ...
      // HRAL filter: skip hounds whose base callsign is not on the paid-member list
      if(hralFilterActive() && !m_hralMembers.contains(Radio::base_callsign(houndCall))) continue;
